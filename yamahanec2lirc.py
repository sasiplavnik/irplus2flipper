...
def yamahanec_code_to_lirc(yncode):
    ...
    if (m is not None and m.group(1) is not None and m.group(2) is not None and m.group(3) is not None):
        ...
        if (m is not None and m.group(1) is not None and m.group(2) is not None):
            ...
            b1s = swap_n_flip(b1)
            b2s = swap_n_flip(b2)
            code = (b1s << 24) | ((0xFF - b1s) <<
                                  16) | (b2s << 8) | ((0xFF - b2s) << 0)
        else:
            # Match "7F01-5B24" style codes
            m = re.match(
                "([0-9a-fA-F]{2})([0-9a-fA-F]{2})-([0-9a-fA-F]{2})([0-9a-fA-F]{2})", yncode)
            if (m is not None and m.group(1) is not None and m.group(2) is not None and m.group(3) is not None and m.group(4) is not None):
                ...
                m = re.match(
                    "([0-9a-fA-F]{2})([0-9a-fA-F]{2})-([0-9a-fA-F]{2})", yncode)
                if (m is not None and m.group(1) is not None and m.group(2) is not None and m.group(3) is not None):
                    b1 = int(m.group(1), 16)
                    b2 = int(m.group(2), 16)
                    b3 = int(m.group(3), 16)
        ...
        for row in csvreader:
            if (row[COL_DEVICE] == device and
                ...
                try:
                    ...
                    print("Failed at %s_%s for zone %s" %
                          (row[COL_FNGRP], row[COL_FNNAME], zone))
                    raise
                ...
                print("%s            %s 0x%08X" %
                      (comment_out, key_name.ljust(30), lirc_code))
        csvfile.close()
